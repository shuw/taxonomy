...
let ever = guests;
try { ever = Math.max(guests, (db.query("select n from counters where name = 'guests_ever'").get() as { n: number } | null)?.n ?? guests); } catch { /* an older database has no counters table */ }
