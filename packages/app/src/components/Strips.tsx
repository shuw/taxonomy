...
const CREDIT_SERIES: Series[] = [
  { id: "credit", label: "Credit on hand at year end", color: PALETTE.amt, value: (y) => y.lines.amtCreditCarryforwardOut!.value },
];
...
function combinedSeries(plan: PlanResult): Series[] {
  ...
  const outOf = (y: PlanResult["years"][number]) => (y.lines.totalTax?.value ?? 0) + (y.lines.exerciseCost?.value ?? 0) + (y.lines.giving?.value ?? 0);
  return [
    ...tax,
    { id: "exercise", label: "Exercise cost", color: PALETTE.exercise, value: (y) => y.lines.exerciseCost?.value ?? 0 },
    { id: "giving", label: "Gifts", color: PALETTE.giving, value: (y) => y.lines.giving?.value ?? 0 },
    { id: "kept", label: "Kept", color: PALETTE.kept, hatch: true, value: (y) => Math.max(0, cashIn(y) - outOf(y)) },
  ];
}
export const CombinedStrip = (p: StripProps) => (
  <ColumnStrip {...p} series={combinedSeries(p.plan)} height={240} totalLabel="Net cash" capLabel={(y) => { const n = y.lines.netCash?.value ?? 0; return `${n < 0 ? "−" : "+"}${usdCompact(Math.abs(n))}`; }}
    capLines={(y) => { const n = y.lines.netCash?.value ?? 0; const g = y.lines.giving?.value ?? 0; const gs = y.lines.givingStock?.value ?? 0; return [...(gs > 0 ? [{ text: `${usdCompact(gs)} in shares given`, className: "giving" }] : []), ...(g > 0 ? [{ text: `${usdCompact(g)} given`, className: "giving" }] : []), { text: n < 0 ? `−${usdCompact(-n)} short` : `${usdCompact(n)} kept`, className: n < 0 ? "neg" : "kept" }, { text: `${usdCompact(y.lines.totalTax?.value ?? 0)} tax`, className: "tax" }]; }}
...
const barWidth = (band: number) => Math.round(Math.min(96, Math.max(28, band * 0.5)));
...
function ColumnStrip({ plan, pinned, focusYear, onFocus, onPick = onFocus, series, height, totalLabel = "Total", capLabel, capLines, marker, boomerang }: StripProps & { series: Series[]; height: number; totalLabel?: string; capLabel?: (y: PlanResult["years"][number]) => string; capLines?: (y: PlanResult["years"][number]) => { text: string; className: string }[]; marker?: (y: PlanResult["years"][number]) => { value: number; label: string } | null}) {
  ...
  const capRows = capLines ? Math.max(1, ...plan.years.map((y) => capLines(y).length)) : 1;
  const m = { top: capLines ? 12 + capRows * 16 : 24, right: 12, bottom: 28, left: 46 };
  ...
  return (
    <div className="chart" ref={ref} onMouseLeave={() => setHover(null)}>
      <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} role="img" aria-label="By year">
        <defs>
          <pattern id="hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="6" height="6" fill={PALETTE.kept} /><line x1="0" y1="0" x2="0" y2="6" stroke="var(--surface)" strokeWidth="2" /></pattern>
        </defs>
        {years.some((y) => y.year === focusYear) && <FocusBand x={m.left + band * years.findIndex((y) => y.year === focusYear)} y={4} w={band} h={m.top + plotH + m.bottom - 2} />}
          ...
          return (
            <g key={y.year}>
              {hasPin && pinnedTotals![i]! > 0 && (
                <TopRounded x={pinX} y={yOf(pinnedTotals![i]!)} w={BAR} h={baseY - yOf(pinnedTotals![i]!)} r={4} fill={PALETTE.pinned} />
              )}
              {segs.map((s, j) => {
              ...
              {totals[i]! > 0 && capLines && capLines(y).map((l, k, all) => <text key={k} className={"cap-label " + l.className} x={hasPin ? cx : barX + BAR / 2} y={yOf(Math.max(totals[i]!, pinnedTotals?.[i] ?? 0)) - 8 - (all.length - 1 - k) * 16} textAnchor="middle">{l.text}</text>)}
              <text className={"year-label" + (y.year === focusYear ? " focus" : "")} x={cx} y={baseY + 20} textAnchor="middle" onClick={() => onPick(y.year)}>{y.year}</text>
              <rect x={m.left + band * i} y={0} width={band} height={m.top + plotH + m.bottom} fill="transparent" onMouseEnter={() => { setHover(i); onFocus(y.year); }} onClick={() => onPick(y.year)} style={{ cursor: "pointer" }} />
            </g>
          );
        })}
      ...
      {hover !== null && (
        <ChartTooltip index={hover} width={width} band={band} left={m.left} height={height}>
          ...
          {capLabel && (years[hover]!.lines.givingStock?.value ?? 0) > 0 && <TooltipRow className="muted" label="Shares given, not cash" value={usd(years[hover]!.lines.givingStock!.value)} />}
...
export function CashStrip({ plan, pinned, focusYear, onFocus, onPick = onFocus }: StripProps) {
  ...
  const { band, ticks, plotH, yOf, baseY } = columnLayout(width, height, years.length, Math.max(1, ...rows.map((r) => Math.max(r.wages + r.proceeds, r.exercise + r.tax + r.giving))), m, MAX_BAND);
  const bar = Math.round(Math.min(72, Math.max(20, band * 0.4)));
  const IN = [{ key: "wages", label: "Salary and bonus", color: PALETTE.regular }, { key: "proceeds", label: "Shares sold", color: PALETTE.surtax }] as const;
  const OUT = [{ key: "exercise", label: "Exercise cost", color: PALETTE.exercise }, { key: "tax", label: "Tax", color: PALETTE.amt }, { key: "giving", label: "Gifts", color: PALETTE.giving }] as const;
  const stack = (x: number, parts: readonly { key: "wages" | "proceeds" | "exercise" | "tax" | "giving"; color: string }[], r: (typeof rows)[number]) => {
    let acc = 0;
