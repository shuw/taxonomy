...
export function FactsModal({ profile, years, tab, onTab, edit, onClose, onOpenIntake }: Props) {
  ...
  const tabs: { id: FactTab; title: string; color: string; summary: string }[] = [
      ...
      <div className="modal wide facts" role="dialog" aria-modal="true" aria-label="Your information">
        ...
        <div className="facts-body">
          <nav className="facts-nav">
            {tabs.map((t) => (
              <button type="button" key={t.id} className={"facts-tab" + (t.id === tab ? " on" : "")} onClick={() => onTab(t.id)}>
          ...
          <div className="facts-pane">
            {tab === "you" && (
              <>
                <Field label="Filing status" wide source={src(["filer", "filingStatus"])}><Segmented options={[...FILING_OPTIONS]} columns={4} value={profile.filer.filingStatus} onChange={(v) => set(["filer", "filingStatus"], v)} /></Field>
                <div className="row3">
                ...
                {home.mortgage
                  ? <>
                      <div className="row3">
                        ...
                        <Field label="Property tax" source={src(["home", "propertyTax"])}><MoneyInput value={home.propertyTax ?? 0} onChange={(n) => set(["home", "propertyTax"], n)} /></Field>
                      </div>
            ...
            {tab === "deductions" && (
              <div className="row3">
                ...
                <Field label="State income tax" hint="leave 0 for a modeled state" source={src(["deductions", "stateIncomeTax"])}><MoneyInput value={ded.stateIncomeTax ?? 0} onChange={(n) => set(["deductions", "stateIncomeTax"], n)} /></Field>
            ...
            {tab === "history" && (
              <>
                ...
                <div className="row4">
                  ...
                  <Field label="Short-term loss" source={src(["carryforwards", "capitalLoss", "shortTerm"])}><MoneyInput value={cf.capitalLoss?.shortTerm ?? 0} onChange={(n) => set(["carryforwards", "capitalLoss", "shortTerm"], n)} /></Field>
                  <Field label="Long-term loss" source={src(["carryforwards", "capitalLoss", "longTerm"])}><MoneyInput value={cf.capitalLoss?.longTerm ?? 0} onChange={(n) => set(["carryforwards", "capitalLoss", "longTerm"], n)} /></Field>
                  <Field label="Charitable" source={src(["carryforwards", "charitable"])}><MoneyInput value={cf.charitable ?? 0} onChange={(n) => set(["carryforwards", "charitable"], n)} /></Field>
                </div>
