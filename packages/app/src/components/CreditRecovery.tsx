...
export function CreditRecoveryView({ r, companyName }: { r: Recovery; companyName?: string }) {
  ...
  const sentence = slices.length === 0
    ? `None of it comes back within the plan${r.projectedYear ? "" : ": no year has regular tax above the minimum to absorb it"}.`
    : r.leftover <= 0
      ? `It comes back ${years.join(", ")}, fully recovered by ${r.projectedYear}.`
      : `${years.join(", ")} come back; ${usdCompact(r.leftover)} is still unrecovered at the end of the plan${r.projectedYear ? `, clearing around ${r.projectedYear} at this pace` : ""}.`;
  return (
    <div className="recovery">
      ...
      <div className="recovery-bar" role="img" aria-label="Credit recovery by year">
        ...
        {r.leftover > 0 && <span className="rec-slice left" style={{ width: `${(r.leftover / total) * 100}%` }} title={`${usdCompact(r.leftover)} unrecovered`}><span>later</span></span>}
      </div>
