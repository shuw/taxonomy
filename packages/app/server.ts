...
const server = Bun.serve({
  ...
  routes: {
    ...
    "/api/agent/desktop": { POST: guard((req) => { const refused = sameOrigin(req); if (refused) return refused; if (authEnabled) return bad("Claude Desktop is set up on your own computer, not on this server"); const r = addToDesktop(storeOf(req)); return "error" in r ? bad(r.error) : Response.json(agentConnection(storeOf(req))); }) },
