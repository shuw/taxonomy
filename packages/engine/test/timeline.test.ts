...
describe("a dated change with an end year", () => {
  test("applies only inside its years; without one it carries on", () => {
    ...
    expect(describeChanges(parseProfile(example), p).join(" ")).toContain("in 2027 only");
...
describe("a gift as a decision", () => {
  test("adds to the year's giving on top of the recurring figure, cash to cash out, shares beside it", () => {
    ...
    expect(describeChanges(base, parseProfile(editProfileText(example, [{ path: ["scenarios", "default", "events"], value: [{ id: "e1", kind: "give", year: 2027, how: "daf", amount: 25_000 }] }]))).join(" ")).toContain("give $25,000 to a donor-advised fund in 2027");
    expect(() => tools.whatIf(base, [{ kind: "give", year: 2027, how: "cash", amount: 0 }])).toThrow("more than 0");
    expect(() => tools.whatIf(base, [{ kind: "give", year: 2040, how: "cash", amount: 1_000 }])).toThrow("not in the plan");
  });

});

describe("gift facts from before gifts were decisions", () => {
  ...
  test("the rewrite happens once: the rewritten text has no entry left, and re-reading adds nothing", () => {
  ...
  test("on top of recurring giving, the converted decision is the amount beyond the recurring figure, and the facts tool refuses the shape", () => {
    const p = parseProfile(editProfileText(example, [
