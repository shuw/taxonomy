...
Deliberately absent: editing facts. Facts change through the information dialog or intake;
a model that misheard "620" as "62" should not be able to change a salary from a chat.
...
1. Tool layer in `packages/engine/src/tools.ts` (pure functions over the engine) with tests.
2. `packages/mcp` server over stdio exposing them, registered with Claude Code; try the six
   example requests above against the real profile.
3. Proposal banner and scenario notes in the app; the "Ask" clipboard affordance.
4. Decide on phase 3 after using phases 1 and 2 for a week.
