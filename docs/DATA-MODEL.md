...
| `timeline` | `{ year, until?, path, value, note? }` | applied cumulatively before each year is computed; `until` bounds it (equal to `year` for a one-year gift or gain; omitted means from `year` on); growth assumptions still compound from the plan start |
